...
def get_name_of_uarts() -> list[str]:
    """
    Gibt eine Liste aller aktiven UART-Geräte im System zurück, z. B. /dev/ttyUSB0, /dev/ttyAMA0, etc.
    """
    uart_patterns = ["/dev/ttyUSB*", "/dev/ttyACM*", "/dev/ttyAMA*"]#, "/dev/serial/by-id/*"]
    found_ports = []

    for pattern in uart_patterns:
        for port in glob.glob(pattern):
            # Prüfe, ob das Gerät lesbar ist
            if os.access(port, os.R_OK | os.W_OK):
                found_ports.append(port)

    if not found_ports:
        print("[Warnung] Keine UART-Geräte gefunden. Bitte schließen Sie ein Gerät an.")
        return []
...
class Dynamixel:
    ...
    def get_name_of_uarts(self) -> list[str]:
        return get_name_of_uarts()
    
    def connect(self):
        if self.config.device_name == '':
            self.config.device_name = self.get_name_of_uarts()[-1]
    ...
    def scan(self, uart_name: str, baudrate: int, min_id: int=-1, max_id: int=-1) -> list[int]:
        found_ids = []


        print(f"[Info] Scanne Bus an {uart_name} mit Baudrate {baudrate}...")

        if (min_id < 0) or (min_id > max_id):
            min_id = self.MIN_ID
        if (max_id < 0) or (max_id < min_id):
            max_id = self.MAX_ID
        # IDs durchprobieren
